...
STRATEGY USED: A * search with heuristic approach

BY: JAIDEV SHARMA(1602 - 18 - 733 - 078)
...
//function to initialise array elements to 0
...
//function to copy the contents of an array to another
...
//function to check whether 2 arrays are equal or not
...
//function to check the empty space block
...
//function to insert element into a queue
...
//function to perform heuristic calculation

int calculate_heuristic(node * nd) {
  int i, j, k, l, h = 0;
  for (i = 0; i <= 2; i++) {
    for (j = 0; j <= 2; j++) {
      if (nd -> a[i][j] != 0) {
...
void next_move(node * nd) {
  ...
  if (y + 1 > 2)
    ...
    if (check_list(nd -> ch4) == 1) {
      ...
      nd -> ch4 -> f = nd -> depth + calculate_heuristic(nd -> ch4) + 1;
      insert_list(nd -> ch4);
      insert_queue(nd -> ch4);
    }
  }
  if (y - 1 < 0)
    ...
    if (check_list(nd -> ch3) == 1) {
      ...
      nd -> ch3 -> f = nd -> depth + calculate_heuristic(nd -> ch3) + 1;
      insert_list(nd -> ch3);
      insert_queue(nd -> ch3);
    }
  }
  if (x + 1 > 2)
    ...
    if (check_list(nd -> ch2) == 1) {
      ...
      nd -> ch2 -> f = nd -> depth + calculate_heuristic(nd -> ch2) + 1;
      insert_list(nd -> ch2);
      insert_queue(nd -> ch2);
    }
  }
  ...
  if (x - 1 < 0)
    ...
    if (check_list(nd -> ch1) == 1) {
      ...
      nd -> ch1 -> f = nd -> depth + calculate_heuristic(nd -> ch1) + 1;
      insert_list(nd -> ch1);
      insert_queue(nd -> ch1);
    }
  }
...
//function to perform A*star search
...
int main() {
  #ifndef ONLINE_JUDGE
  freopen("C:\\Users\\Jaidev Sharma\\Desktop\\Interview_Codes\\input.txt", "r", stdin);
  freopen("C:\\Users\\Jaidev Sharma\\Desktop\\Interview_Codes\\output.txt", "w", stdout);
  #endif
  node * n;
  int i, j;
